...
export function DeviceManager({ zones }: { zones: Zone[] }) {
  ...
  // Seçilen zone için bir sonraki deviceId önerisi: {slug}-00N
  const suggestId = useMemo(() => {
    return (slug: string) => {
      const count = devices.filter((d) => d.zoneSlug === slug).length;
  ...
  function openAdd() {
    ...
    setZoneSlug(firstZone);
    setDeviceId(firstZone ? suggestId(firstZone) : "");
    setName("");
    setError(null);
    setAddOpen(true);
  }

  function onZoneChange(slug: string) {
    setZoneSlug(slug);
    setDeviceId(slug ? suggestId(slug) : "");
  }

  async function submitAdd(e: React.FormEvent) {
    e.preventDefault();
    if (!deviceId.trim() || !zoneSlug) return;
    setSubmitting(true);
    setError(null);
    try {
      const res = await fetch("/api/devices", {
        ...
        body: JSON.stringify({ deviceId: deviceId.trim(), zoneSlug, name: name.trim() || undefined }),
      });
    ...
    <section aria-label="Cihazlar">
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Yeni Cihaz">
        ...
        <form onSubmit={submitAdd} className="flex flex-col gap-4">
          ...
          <div>
            <label className={labelCls} htmlFor="dv-id">Cihaz kimliği (deviceId) *</label>
            <input
              id="dv-id"
              className={`${inputCls} font-mono`}
              value={deviceId}
              onChange={(e) => setDeviceId(e.target.value)}
              placeholder="ataturk-bulvari-001"
            />
            <p className="mt-1 text-[11px] text-muted">
              ESP32 firmware&apos;ındaki <code>DEVICE_ID</code> ile birebir aynı olmalı.
            </p>
          </div>
          <div>
          ...
          <div className="mt-1 flex justify-end gap-2">
            ...
            <button type="submit" disabled={submitting || !deviceId.trim()} className="rounded-lg border border-glow/40 bg-glow/20 px-4 py-2 text-sm font-semibold text-text transition-colors hover:bg-glow/30 disabled:opacity-50">
      ...
      <Modal open={deleting !== null} onClose={() => setDeleting(null)} title="Cihaz sil">
        <p className="text-sm text-muted">
          <span className="font-mono text-text">{deleting?.deviceId}</span> cihazını silmek istediğine emin misin?
        </p>
